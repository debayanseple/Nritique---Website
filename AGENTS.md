...
- Workshop listings are fetched client-side from public Google Sheets via `gviz/tq?tqx=out:csv` (no auth, CORS-enabled). Hardcoded sheet IDs live in `src/lib/api/workshops.functions.ts`. Rows are matched by header name (case-insensitive) via the `col()` helper: `Workshop image` (falls back to the legacy `Cover Photo` header), `Status?`, `Seats left`, `Price (INR)`, `Registration link`, `Workshop title`. Renaming a sheet header silently breaks the map (status becomes empty, so workshops drop out). Drive links in the image column are converted to `drive.google.com/thumbnail?...` URLs.
